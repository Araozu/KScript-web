...
div.prism-editor-wrapper
    // line-numbers
    div.prism-editor__line-numbers(:style="{'min-height': lineNumbersHeight}" ref="refLineNumbers")
        // lineNumberWidthCalculator
        div.prism-editor__line-width-calc(style="height: 0px; visibility: hidden; pointer-events: none;")
    ...
    div.prism-editor__container
        ...
        // preview
        pre.prism-editor__editor(ref="refPreview" data-testid="preview" v-html="content")
...
export default defineComponent({
    ...
    setup(props, context) {
        ...
        const content = computed(() => highlight(codeData.value) + '<br />');
        const lineNumbersCount = computed(() => codeData.value.split(/\r\n|\n/).length);
        ...
        const styleLineNumbers = () => {
            ...
            const $lineNumbers = refLineNumbers.value;
        ...
        const _recordCurrentState = () => {
            const input = refTextArea.value;
        ...
        const _getLines = (text, position) => text.substring(0, position).split('\n');

        const _applyEdits = (record) => {
        ...
        const _recordChange = (record, overwrite = false) => {
        ...
        const textarea = h('textarea', {
            ...
            class: {
                'prism-editor__textarea--empty': isEmpty.value,
            },
            spellCheck: 'false',
            autocapitalize: 'off',
            autocomplete: 'off',
